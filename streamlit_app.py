...
def main():
    ...
    st.markdown(
        ...
        /* Download button animation: pulsing effect */
        @keyframes pulse {{
            0% {{ transform: scale(1); }}
    ...
    if st.button("Anonymize Document"):
        ...
        try:
            ...
            # Read the anonymized file as bytes for download
            with open(output_path, "rb") as docx_file:
                anonymized_bytes = docx_file.read()

            # Create a download button with the pulsing animation
            st.download_button(
                label="Download Anonymized DOCX",
